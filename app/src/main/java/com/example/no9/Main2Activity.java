...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final Button button = (Button) findViewById(R.id.ContinueToGuess);

        Intent intent=getIntent();
        random = intent.getIntExtra("Random",0);
        input_N = intent.getIntExtra("InputNumber",0);
        guessTime = intent.getIntExtra("GuessTime",0);

        TextView text_prompt = (TextView)findViewById(R.id.Prompt);
        TextView text_result = (TextView)findViewById(R.id.Result);
        if (random == input_N){
            text_result.setText("O");
            guessTime++;
            text_prompt.setText("Bingo!!"+guessTime+"次");
            ReturnGame=true;
            guessTime=0;
            button.setText("再玩一次");
        }
